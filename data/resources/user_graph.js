...
window.onload = function () {
    ...
    for (var i = 0; i <= 10; i+=2) {
        var scaled = (1 - (i / 10.0)) * max;
        var minutes = parseInt(scaled / 1000 / 60);
        var hours = parseInt(minutes/ 60);
        minutes -= (hours * 60);
        r.text(Math.round(leftgutter + X * .5 + .5 - 25) + 0.5, Math.round(topgutter + .5 + i * rowHeight), hours + ":" + (minutes<10?"0":"") + minutes).attr(txt).toBack();
    }
    ...
    var path = r.path().attr({stroke: color, "stroke-width": 4, "stroke-linejoin": "round"}),
        ...
        leave_timer,
        blanket = r.set();
    label.push(r.text(60, 12, "0hrs, 0mins").attr(txt));
    label.push(r.text(60, 27, "00/00").attr(txt1).attr({fill: color}));
    label.hide();
